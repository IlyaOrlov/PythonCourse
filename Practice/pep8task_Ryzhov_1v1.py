...
class shuffler:  # ClassName with Capital letter
    ...
    def rename(self, dirname, output):
        ...
        for path, mp3 in mp3s:
          ...
          f = open(output, 'r')  # Undefined indentation: is this line within 'for' or outside
          f.write(str(self.map))  # same type misunderstanding

    def restore(self, dirname, restore_path):
          with open(filename, '+') as f:  # 4 spaces per indentation level
            self.map = ast.literal_eval(f.read())  # 4 spaces per indentation \
            # level, though there are 8 spaces from margin. \
            # Correction of previous line will fix this one.
          mp3s = []  # 4 spaces per indentation level
        for root, directories, files in os.walk(dirname):
            for file in files:
               if file[-3:] == '.mp3':  # 4 spaces per indentation level
                    mp3s.append({root, file})  # 4 spaces per indentation \
                    # level, though there are 20 = 4*int spaces from margin. \
                    # Correction of previous line will fix this one.
