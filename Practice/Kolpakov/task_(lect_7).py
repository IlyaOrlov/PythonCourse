...
class Human:
    ...
    def __repr__(self):
        return f'{self.name}, {self.age} лет, страна рождения: {self.location}, профессия: {self.profession}, мое ' \
               f'хобби: {self.hobby}\n '
...
profession = ['Хирург', 'Пилот', 'Исследователь', 'Космонавт', 'Продюсер', 'Биохимик', 'Частный детектив', 'Археолог',
              'Дипломат', 'Кондитер', 'Корреспондент']
hobby = ['Охота', 'Охота', 'Ролевые игры', 'Пейнтбол', 'Садоводство', 'Кулинария', 'Коллекционирование',
         'Компьютерные игры', 'Чтение', 'фотография']
...
def new_person(kol):
    ...
    for i in range(kol):
        humans.append(
            Human(random.choice(name), random.randint(20, 65), random.choice(location), random.choice(profession),
                  random.choice(hobby)))
