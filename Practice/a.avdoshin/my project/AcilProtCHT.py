# 22.11 - [ИО]:  Проверено (есть замечания)
# 1. Работать с файлами без менеджера контекста небезопасно.
# 2. много "магических чисел" - программу трудно будет переиспользовать
# потом и понимать сейчас; все используемые в явном виде числа
# желательно заменить на константы, определяемые в отдельном фале
# или в начале этого файла.
# 3. Почему вы используете именно двойное подчеркивание в именах атрибутов?
# 4. (на будущее, когда пройдем соответствующую тему) Хорошая программа должна
# сопровождаться набором тестов (как минимум, unit-тестов).
import math
import sys
import random
